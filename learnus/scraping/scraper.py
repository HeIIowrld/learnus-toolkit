...
class LearnUsScraper:
    ...
    def _get(self, url: str, timeout: int = 20, stream: bool = False, retries: int = 1):
        """Perform a GET request with a light retry for transient timeouts."""
        last_error = None
    ...
    def parse_course_list(self, year: str = None, semester: str = None) -> List[CourseInfo]:
        ...
        try:
            ...
            print(f"[INFO] Making HTTP GET request to: {url}")
            print(f"[INFO] Session cookies: {list(self.session.cookies.keys())}")
            
            response = self._get(url, timeout=20, retries=1)
            
            print(f"[OK] Response status: {response.status_code}")
            print(f"[OK] Response URL (after redirects): {response.url}")
            ...
            if soup.select_one('tbody.my-course-lists'):
                print("[INFO] Table view detected")
                courses = self._parse_table_course_list(soup)
                print(f"[OK] Found {len(courses)} courses in table view")
                return courses

            # 4. Fallback: Card View (Main Dashboard)
            print("[INFO] Checking for card view (Dashboard)...")
            ...
            print(f"[OK] Total courses found: {len(courses)}")
            return courses

        except Exception as e:
            print(f"[ERROR] Error parsing course list: {e}")
            return []
    ...
    def get_course_name(self, course_url: str) -> str:
        ...
        try:
            response = self._get(course_url, timeout=20, retries=1)
            soup = BeautifulSoup(response.text, 'html.parser')
    ...
    def parse_lecture_list(self, course_url: str) -> List[LectureInfo]:
        ...
        try:
            ...
            for activity_div in activity_instances:
                ...
                # Generic Moodle icon paths contain "icon", so require a real video hint.
                    ...
                    signal_text = f"{href} {onclick} {src} {activity_text}".lower()
                    if any(x in signal_text for x in [
                        'video',
                        'vod',
                        'play',
                        'movie',
            ...
            print(f"  [OK] Found {len(lectures)} lectures")
            return lectures
        except Exception as e:
    ...
    def parse_course_content(self, course_id):
        ...
        try:
            ...
            print(f"\n  [INFO] Parsing course content from: {course_url}")
            ...
            for section_elem in section_elements:
                ...
                for activity in activities:
                    ...
                    # 4. Board/resource containers that may have attachments.
                    elif 'mod/ubboard' in href:
                        materials.append({
                            'name': activity_name,
                            'url': href,
                            'type': 'board',
                    ...
                    # 5. Page/Resource types that might contain downloadable files
                    # Syllabus, course info pages, etc. - treat as materials if they seem file-like
                    elif 'mod/page' in href or 'mod/book' in href:
                    ...
                    # 6. Any link with file extensions in URL or name should be considered
            ...
            print(f"  [OK] Found {len(sections)} sections with materials/assignments")
            if professor:
                print(f"  [OK] Professor: {professor}")
            return {'sections': sections, 'professor': professor}
            
        except Exception as e:
            print(f"  [ERROR] Error parsing course content: {e}")
    ...
    def extract_video_url(self, lecture: LectureInfo) -> Optional[str]:
        ...
        try:
            ...
            print(f"    [INFO] Extracting video URL from: {lecture.activity_url}")
            response = self._get(lecture.activity_url, timeout=30, retries=1)
            response.raise_for_status()
            
            html_content = response.text
            soup = BeautifulSoup(html_content, 'html.parser')

            if re.search(r'VOD does not exist|동영상이 존재하지', html_content, re.IGNORECASE):
                self.last_error = "VOD does not exist on LearnUs server"
            ...
            for source in sources:
                ...
                if '.m3u8' in src or 'mpegURL' in src_type or 'mpegurl' in src_type:
                    ...
                    print(f"    [OK] Found m3u8 URL: {video_url}")
                    return video_url
                
                if src.endswith('.mp4') or 'mp4' in src_type:
                    video_url = src if src.startswith('http') else f"https:{src}"
                    print(f"    [OK] Found mp4 URL: {video_url}")
                    return video_url
            ...
            for video in videos:
                ...
                if src.endswith('.mp4') or src.endswith('.m3u8') or '.m3u8' in src:
                    video_url = src if src.startswith('http') else f"https:{src}"
                    print(f"    [OK] Found video tag URL: {video_url}")
                    return video_url
            ...
            if m3u8_match:
                ...
                print(f"    [OK] Found m3u8 URL (regex): {video_url}")
                return video_url
            ...
            if mp4_match:
                ...
                print(f"    [OK] Found mp4 URL (regex): {video_url}")
                return video_url
            ...
            for pattern in js_patterns:
                ...
                if match:
                    ...
                    print(f"    [OK] Found video URL (JS variable): {video_url}")
                    return video_url

            if 'mod/url/view.php' in lecture.activity_url and 'redirect=1' not in lecture.activity_url:
                redirect_url = lecture.activity_url + ('&' if '?' in lecture.activity_url else '?') + 'redirect=1'
                redirected = self._get(redirect_url, timeout=30, retries=1)
                redirected.raise_for_status()
                if redirected.url != lecture.activity_url and '/mod/vod/' in redirected.url:
                    redirected_lecture = LectureInfo(
                        lecture.lecture_id,
                        lecture.title,
                        lecture.week,
                        lecture.status,
                    ...
                    return self.extract_video_url(redirected_lecture)
            
            print(f"    [ERROR] No video URL found in viewer page")
            self.last_error = "Could not extract video URL"
            return None
            
        except Exception as e:
            self.last_error = f"Error extracting video URL: {e}"
            print(f"    [ERROR] {self.last_error}")
            return None
    ...
    def download_file(self, url: str, path: str) -> bool:
        ...
        try:
            ...
            print(f"    [INFO] Downloading: {save_path.name}")
            ...
            def normalize_url(href: str) -> str:
                ...
                if href.startswith('http'):
                    return href
                return f"{LEARNUS_ORIGIN}{href}" if href.startswith('/') else f"{LEARNUS_ORIGIN}/{href}"

            def convert_external_download_url(href: str) -> str:
                colab_match = re.search(r'colab\.research\.google\.com/drive/([^?#/]+)', href, re.I)
                if colab_match:
                    file_id = colab_match.group(1)
                    return f"https://drive.google.com/uc?export=download&id={file_id}"
                return href

            def find_download_link(html: str, base_url: str, desired_suffix: str = '') -> Optional[str]:
                ...
                preferred_patterns = [
                    ...
                    r'\.(pdf|docx?|pptx?|xlsx?|zip|rar|7z|py|r|c|cpp|h|java|txt|csv|ipynb|hwp|hwpx)(?:[?#]|$)',
                ]
                for pattern in preferred_patterns:
                    for link in links:
                        href = link.get('href', '')
                        if re.search(pattern, href, re.I):
                            return convert_external_download_url(normalize_url(href))
                ...
                if desired_suffix:
                    for link in links:
                        ...
                        if desired_suffix.lower() in combined:
                            return convert_external_download_url(normalize_url(href))
                        if desired_suffix.lower() == '.ipynb' and 'colab.research.google.com/drive/' in href:
                            return convert_external_download_url(normalize_url(href))

                return None

            # Make request with session to maintain authentication
            response = self._get(url, timeout=30, stream=True, retries=1)
            response.raise_for_status()

            content_type = response.headers.get('Content-Type', '')
            final_url = response.url or url
            suffix = save_path.suffix.lower()
            html_like = 'text/html' in content_type.lower()

            if html_like and suffix not in {'.html', '.htm'}:
                html_response = self._get(final_url, timeout=30, stream=False, retries=1)
                html_response.raise_for_status()
                if 'login' in html_response.url.lower() or '로그인' in html_response.text[:1000]:
                    self.last_error = f"Authentication page returned instead of file: {url}"
                    print(f"    [ERROR] {self.last_error}")
                    return False
                direct_url = find_download_link(html_response.text, final_url, suffix)
                if direct_url and direct_url != final_url:
                    response.close()
                    response = self._get(direct_url, timeout=30, stream=True, retries=1)
                    response.raise_for_status()
                    content_type = response.headers.get('Content-Type', '')
                    html_like = 'text/html' in content_type.lower()
                if html_like and suffix not in {'.html', '.htm'}:
                    self.last_error = f"HTML page returned instead of file: {final_url}"
                    print(f"    [ERROR] {self.last_error}")
                    return False
            ...
            if file_size == 0:
                ...
                self.last_error = f"Downloaded file is empty: {url}"
                print(f"    [ERROR] {self.last_error}")
                return False
            ...
            try:
                partial_path = Path(path).with_name(f"{Path(path).name}.part")
                partial_path.unlink(missing_ok=True)
            except Exception:
                pass
    ...
    def parse_assignment_page(self, url: str) -> Dict:
        ...
        """
        try:
            print(f"    [INFO] Parsing assignment page: {url}")
            response = self._get(url, timeout=20, retries=1)
            soup = BeautifulSoup(response.text, 'html.parser')
            ...
            def clean_filename(value: str) -> str:
                ...
                return "".join(c for c in value if c not in r'<>:"/\|?*').strip()

            def filename_from_link(link, href: str) -> str:
                filename = link.get_text(strip=True)
                img = link.find_previous('img')
                if (not filename or len(filename) > 200) and img:
                    filename = img.get('alt') or img.get('title') or ''
                if not filename or len(filename) > 200:
                    url_part = href.split('/')[-1].split('?')[0]
                    filename = unquote(url_part) if url_part else ''
                if not filename:
                    match = re.search(r'[?&]file=([^&]+)', href)
                    filename = unquote(match.group(1)) if match else ''
                return clean_filename(filename) or 'assignment_file'

            def is_download_link(href: str) -> bool:
                if not href:
                    return False
                if re.search(r'(pluginfile\.php|mod/resource|forcedownload=1|download=1)', href, re.I):
                    return True
                return bool(re.search(r'\.(pdf|docx?|pptx?|xlsx?|zip|rar|py|r|c|cpp|java|txt|html|css|js)(?:\?|$)', href, re.I))

            def is_submission_link(link, href: str) -> bool:
                if re.search(r'assignsubmission_file|submission_files', href, re.I):
                    return True
                ...
                for _ in range(8):
                    ...
                    if re.search(r'assignsubmission|submissionstatus|submissionsummary|fileuploadsubmission', class_text, re.I):
                        return True
                    if re.search(r'제출물|제출 파일|첨부파일|제출 상황|Submission|Submitted files?', text, re.I):
                        return True
                    node = getattr(node, 'parent', None)

                return False

            # Extract assignment description text. LearnUs/Moodle keeps the assignment
            # prompt and instructor attachments in #intro.
            ...
            for link in all_links:
                ...
                if not is_download_link(raw_href):
                    continue
                ...
                filename = filename_from_link(link, href)
                if filename.lower() in ['download', 'view', 'open', 'link', 'here', 'click']:
                    continue

                target = submissions if is_submission_link(link, href) else requirements
                target.append({
                    'name': filename,
                    'url': href
                })
            
            print(f"    [OK] Found {len(requirements)} requirement files, {len(submissions)} submissions")
            if description_text:
                print(f"    [OK] Extracted description ({len(description_text)} chars)")
            ...
            print(f"    [ERROR] Error parsing assignment page: {e}")
    ...
    def parse_folder_page(self, url: str) -> Dict:
        ...
        """
        try:
            print(f"    [INFO] Parsing folder page: {url}")
            response = self._get(url, timeout=20, retries=1)
            soup = BeautifulSoup(response.text, 'html.parser')
            ...
            print(f"    [ERROR] Error parsing folder page: {e}")
            return {'files': [], 'description': ''}

    def parse_board_page(self, url: str) -> Dict:
        """
        Parse a LearnUs board page and collect direct attachments from visible posts.
        Empty boards are valid containers and should not be treated as failed files.
        """
        try:
            print(f"    [INFO] Parsing board page: {url}")
            response = self._get(url, timeout=20, retries=1)
            soup = BeautifulSoup(response.text, 'html.parser')
            ...
            def filename_from_link(link, href: str) -> str:
                filename = link.get_text(' ', strip=True)
                if not filename or len(filename) > 200:
                    filename = unquote(href.split('?', 1)[0].rstrip('/').rsplit('/', 1)[-1])
                return re.sub(r'\s+', ' ', filename or 'board_attachment').strip()

            def collect_from_page(page_soup: BeautifulSoup) -> None:
                for link in page_soup.find_all('a', href=True):
                    href = link.get('href', '')
                    if not re.search(
                        r'(pluginfile\.php|forcedownload=1|\.(pdf|docx?|pptx?|xlsx?|zip|rar|7z|py|r|c|cpp|h|java|txt|csv|ipynb|hwp|hwpx)(?:[?#]|$))',
                        href,
                        re.I,
                    ):
                        continue
                    ...
                    seen.add(full_url)
                    files.append({
                        'name': filename_from_link(link, full_url),
                        'url': full_url,
                    })

            collect_from_page(soup)

            post_links = []
            for link in soup.find_all('a', href=True):
                href = normalize_url(link.get('href', ''))
                if href and 'mod/ubboard' in href and re.search(r'(bwid|article|post|view\.php.*&)', href, re.I):
                    if href not in post_links:
                        post_links.append(href)

            for post_url in post_links[:50]:
                try:
                    post_response = self._get(post_url, timeout=20, retries=1)
                    post_soup = BeautifulSoup(post_response.text, 'html.parser')
                    collect_from_page(post_soup)
                except Exception as exc:
                    print(f"    [WARN] Board post parse failed: {exc}")

            board_text = soup.select_one('.ubboard_container, .ubboard')
            if board_text:
                description_text = board_text.get_text('\n', strip=True)

            print(f"    [OK] Found {len(files)} files in board")
            return {
                'files': files,
                'description': description_text,
            }
        except Exception as e:
            print(f"    [ERROR] Error parsing board page: {e}")
            return {'files': [], 'description': '', 'error': str(e)}
