...
from migrate_downloads import migrate_downloads
import threading
import time
...
def download_lectures():
    ...
    def download_task():
        try:
            ...
            # Local processing always available in local version
            transcriber = WhisperTranscriber() if (transcribe_ids or summarize_audio_only or analyze_video) else None
            summarizer = Summarizer() if (summarize_ids or summarize_audio_only) else None
            video_analyzer = VideoAnalyzer() if analyze_video else None
...
def migrate():
    """Manually trigger migration of old directory structure to new structure"""
    try:
        # Run migration
        migrate_downloads(download_dir, dry_run=False)
        
        return jsonify({
            'success': True,
            'message': 'Migration completed successfully. Old files have been reorganized into the new directory structure.'
        })
    except Exception as e:
        import traceback
        return jsonify({
